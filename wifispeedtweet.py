            ...
            if self.config.driver_type is not None:
                ...
                if self.config.driver_type == DriverType.firefox:
                    ...
                    if self.config.binary_path is not None:
                        self.driver = webdriver.Firefox(FirefoxBinary("C:\Program Files (x86)\Mozilla Firefox\firefox.exe"))
                    else:
                        self.driver = webdriver.Firefox()
                # Create a Chrome driver (no specific executable needed)
                elif self.config.driver_type == DriverType.chrome:
                        self.driver = webdriver.Chrome(executable_path="/mnt/d/PersonalProjects/chromedriver_win32/chromedriver.exe")
...
class Output(object):
    ...
    def test_results(self):
        ...
        if (float(self.speedtest_download) < self.config_download * self.margin or
                ...
                try:
                    self.twitter.api.update_status(" I pay for " +
                                           str(self.config_download) + " Mbps download and " +
                                           str(self.config_upload) + " Mbps upload. Why am I then only getting " +
            ...
            self.log.write_to_log("-- " + self.config.date + " --\n"
                                  ...
                                  "-------------------- \n")
            self.twitter.api.update_status("yay... I'm getting " +
                                    self.speedtest_download + "Mbps download and " +
                                    self.speedtest_upload + "Mbps upload today.")
